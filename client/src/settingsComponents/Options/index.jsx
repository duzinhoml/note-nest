            ...
            <div className="container-fluid d-flex flex-column">
                <ul className="list-group list-group-flush">
                    <li className={`list-group-item ml-background ${settingsSelection === 'updatePassword' ? 'settings-options d-flex justify-content-between' : ''} text-light rounded settings-interact border-bottom-0`} onClick={() => toggleSettingsSelection('updatePassword')}>
                        <div>
                            <span className="me-2"><i className="fa-solid fa-lock" style={{ color: settingsSelection === 'updatePassword' ? '#F63366' : '#f8f9fa' }}></i></span>
                            Change Password
                        </div>
                        {settingsSelection === 'updatePassword' ? (<span><i className="fa-solid fa-chevron-right" style={{ color: '#F63366' }}></i></span>) : ''}
                    </li>
                    <li className={`list-group-item ml-background ${settingsSelection === 'deleteUser' ? 'settings-options d-flex justify-content-between' : ''} text-light rounded settings-interact border-bottom-0`} onClick={() => toggleSettingsSelection('deleteUser')}>
                        <div>
                            <span className="me-2"><i className="fa-solid fa-user" style={{ color: settingsSelection === 'deleteUser' ? '#F63366' : '#f8f9fa', marginLeft: '1px' }}></i></span>
                            Delete Account
                        </div>
                        {settingsSelection === 'deleteUser' ? (<span><i className="fa-solid fa-chevron-right" style={{ color: '#F63366' }}></i></span>) : ''}
                    </li>
                    ...
                    <li className={`list-group-item text-light rounded settings-logout border-bottom-0`} onClick={() => Auth.logout()}>
                        <span className="me-2" style={{ marginLeft: '1px' }}><i className="fa-solid fa-right-from-bracket"></i></span>
                        Logout
                    </li>
                </ul>
            </div>
